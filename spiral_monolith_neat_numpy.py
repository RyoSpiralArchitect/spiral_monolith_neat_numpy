...
class LCSMonitor:
    ...
    def _reachable_from_inputs(self, adj, inputs):
        seen = set()
        queue = deque()
        for s in inputs:
            if s in adj:
    ...
    def _count_paths_DAG(self, order, parents, inputs):
    ...
    def _shortest_len_DAG(self, order, parents, inputs):
    ...
    def _strongly_connected_components(self, adj):
        ...
        comp_nodes = {idx: nodes for idx, nodes in enumerate(components)}
        return comp_map, comp_nodes

    def _build_component_graph(self, adj, comp_map, comp_count):
        comp_adj = {i: [] for i in range(comp_count)}
        seen = set()
        for u, nbrs in adj.items():
            cu = comp_map[u]
            for v, _ in nbrs:
                ...
                if cu != cv and (cu, cv) not in seen:
                    comp_adj[cu].append((cv, 1))
                    seen.add((cu, cv))
        return comp_adj

    def _component_inputs(self, comp_map):
        comp_inputs = set()
        for node in self.inputs:
            if node in comp_map:
                comp_inputs.add(comp_map[node])
        return comp_inputs

    def _internal_component_distances(self, adj, comp_map, comp_nodes):
    ...
    def _analyze_graph(self, adj):
        comp_map, comp_nodes = self._strongly_connected_components(adj)
        comp_adj = self._build_component_graph(adj, comp_map, len(comp_nodes))
        try:
            order, parents = self._topo_order(comp_adj)
        except ValueError:
            # Should not happen, but guard against residual cycles from numerical issues
            order, parents = [], {}
        comp_inputs = self._component_inputs(comp_map)
        reach_comp = self._reachable_from_inputs(comp_adj, comp_inputs)
        paths_comp = self._count_paths_DAG(order, parents, comp_inputs)
        dist_comp = self._shortest_len_DAG(order, parents, comp_inputs)
        internal = self._internal_component_distances(adj, comp_map, comp_nodes)
        reachable = set()
        paths = {}
        dist = {}
        for node, comp in comp_map.items():
            internal_dist = internal.get(comp, {}).get(node, INF)
            if comp in reach_comp and internal_dist < INF:
                reachable.add(node)
            paths[node] = paths_comp.get(comp, 0)
            base = dist_comp.get(comp, INF)
            if base >= INF or internal_dist >= INF:
                dist[node] = INF
            else:
                dist[node] = base + internal_dist
        return reachable, paths, dist

...
class LCSMonitor:
    ...
    def _complete_adj(self, adj):
        ...
        return {u: list(adj.get(u, ())) for u in nodes}

    def _scc_condensation(self, adj):
        graph = self._complete_adj(adj)
        index = 0
        stack = []
        indices = {}
        lowlink = {}
        ...
        def strongconnect(v):
            ...
            stack.append(v)
            on_stack.add(v)
            for w, _ in graph.get(v, ()):  # pragma: no branch - tiny helper
                if w not in indices:
                    strongconnect(w)
        ...
        cond_adj = {cid: set() for cid in comp_nodes}
        incoming = {cid: [] for cid in comp_nodes}
        internal = {cid: {node: [] for node in comp_nodes[cid]} for cid in comp_nodes}
        cyclic = set(cid for cid, comp in comp_nodes.items() if len(comp) > 1)

        for u, nbrs in graph.items():
            cu = node_to_comp[u]
            internal.setdefault(cu, {}).setdefault(u, [])
            for v, _ in nbrs:
                cv = node_to_comp[v]
                if cu == cv:
                    internal[cu].setdefault(u, []).append(v)
                    internal[cu].setdefault(v, [])
                    if u == v:
                        cyclic.add(cu)
                else:
                    cond_adj[cu].add(cv)
                    incoming[cv].append((cu, u, v))

        # Build adjacency suitable for topo ordering
        cond_for_topo = {cid: [(nbr, 1.0) for nbr in cond_adj.get(cid, set())] for cid in comp_nodes}

        return graph, node_to_comp, comp_nodes, cond_adj, cond_for_topo, incoming, internal, cyclic

    def _analyze_graph(self, adj):
        graph, node_to_comp, comp_nodes, cond_adj, cond_for_topo, incoming, internal, cyclic = self._scc_condensation(adj)

        reach = self._reachable_from_inputs(graph)

        order, _ = self._topo_order(cond_for_topo)

        paths_comp = {cid: 0 for cid in comp_nodes}
        for s in self.inputs:
            if s in node_to_comp:
                cid = node_to_comp[s]
                paths_comp[cid] = min(self.K, paths_comp[cid] + 1)

        for cid in order:
            count = paths_comp.get(cid, 0)
            if count == 0:
                continue
            if cid in cyclic:
                count = self.K
                paths_comp[cid] = self.K
            for nbr in cond_adj.get(cid, set()):
                if cid in cyclic or nbr in cyclic:
                    paths_comp[nbr] = self.K
                else:
                    paths_comp[nbr] = min(self.K, paths_comp.get(nbr, 0) + count)

        paths = {node: min(self.K, paths_comp.get(node_to_comp.get(node, -1), 0)) for node in graph}

        dist_comp = {cid: INF for cid in comp_nodes}
        for s in self.inputs:
            if s in node_to_comp:
                dist_comp[node_to_comp[s]] = 0

        for cid in order:
            base = dist_comp.get(cid, INF)
            if base >= INF:
                continue
            for nbr in cond_adj.get(cid, set()):
                cand = base + 1
                if cand < dist_comp.get(nbr, INF):
                    dist_comp[nbr] = cand
        ...
        for cid, nodes in comp_nodes.items():
            ...
            for node in nodes:
                if node in self.inputs:
                    entry.add(node)
            for pred, _, v in incoming.get(cid, ()):  # pragma: no branch - simple accumulation
                if dist_comp.get(pred, INF) < INF:
                    entry.add(v)
            if not entry:
                continue
            local_dist = {node: INF for node in nodes}
            dq = deque()
            for node in entry:
                local_dist[node] = 0
                dq.append(node)
            while dq:
                u = dq.popleft()
                for v in internal.get(cid, {}).get(u, ()):  # pragma: no branch - small helper
                    if local_dist[v] > local_dist[u] + 1:
                        local_dist[v] = local_dist[u] + 1
                        dq.append(v)
            for node in nodes:
                extra = local_dist.get(node, INF)
                if extra < INF:
                    dist[node] = min(dist[node], base + extra)

        return reach, paths, dist
    ...
    def _compute_metrics(self, G0, G1, changed_nodes, changed_edges, lineage_id, gen, mut_id):
        ...
        reach0, paths0, dist0 = self._analyze_graph(adj0s)
        reach1, paths1, dist1 = self._analyze_graph(adj1s)
